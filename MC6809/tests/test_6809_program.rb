...
#         super(Test6809_Program, self).setUp()
    ...
    def _crc16 (data)
        ...
        @cpu.user_stack_pointer.set(data_address) # start address of data
        @cpu.index_x.set(len(data)) # number of bytes
        ...
        crc16 = @cpu.accu_d.value
        return crc16
    end
    
    def test_crc16_01
        crc16 = _crc16("Z")
        assertEqualHex(crc16, 0xfbbf)
    end
    
    def test_crc16_02
        crc16 = _crc16("DragonPy works?!?")
        assertEqualHex(crc16, 0xA30D)
    end
...
#        print "\tcrc32...:", hex2
    ...
    def test_crc32_01
        _test_crc32("a09") # $3617c6fe
    end
    
    def test_crc32_02
        _test_crc32("DragonPy test!") # $570e3666
    end
    
    def test_crc32_03
        _test_crc32("ZYXWVUTSRQPONMLKJIHGFEDBCA") # $99cdfdb2
    end
    
    # following tests works too but takes some time to run
end
#    def test_crc32_04(self)
#        _test_crc32("DragonPy Integration testing...") # $728b1186
#    def test_crc32_05(self)
#        _test_crc32("An Arbitrary String") # $6fbeaae7
#    def test_crc32_06(self)
#        _test_crc32("ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz 0123456789") # $749f0b1a
    
    def _division (dividend, divisor)
...
#         print "b:", [hex(i) for i in b]
        ...
        quotient = @cpu.index_x.value
        remainder = @cpu.accu_d.value
    ...
    def test_division
        def test (dividend, divisor)
            ...
            """
            quotient, remainder = _division(dividend, divisor)
        end
    end
end
#             print quotient, remainder
            ...
            first = sprintf("%i/%i=%i remainder: %i", dividend, divisor, quotient, remainder)
            second = sprintf("%i/%i=%i remainder: %i", dividend, divisor, expected_quotient, expected_remainder)
        end
    end
end
#             print first
...
#         print "b:", [hex(i) for i in b]
        ...
        Special cases
           1. overflow: quotient overflow if dividend.equal? to great(remainder = divisor),
               remainder.equal? set to $FFFF -> special handling.
               This.equal? checked also right before the main loop.
           end
        ...
        cpu_test_run(start=0x0000, end_=nil, mem=[
            ...
            0x8E, 0xFF, 0xFF, # 003F|          LDX   #$FFFF  ; quotient = FFFF.new(-1) marks overflow
            #                   0042|                        ; (case 1)
            #                   0042|  UMMOD4
...
#             print sprintf("$%x / $%x", dividend, divisor)
            first = sprintf("%i/%i=%i remainder: %i (hex: q:$%x r:=$%x)", 
                dividend, divisor, quotient, remainder,
                quotient, remainder,
            end
            )
            second = sprintf("%i/%i=%i remainder: %i (hex: q:$%x r:=$%x)", 
...
#             if first != second
#                 print sprintf("ERROR: %r should be: %r\n", first, second)
#             else
#                 print "OK: %s\n" % first
            assertEqual(first, second)
        end
    ...
    def test_overflow
        """
        overflow(quotient.equal? > $FFFF)
        quotient = $FFFF, remainder = divisor
        """
        quotient, remainder = _division(0x10000, 0x1)
        assertEqualHexWord(quotient, 0xffff)
        assertEqualHexWord(remainder, 0x1)
    end
    
    def test_division_by_zero
        quotient, remainder = _division(0x1, 0x0)
        assertEqualHexWord(quotient, 0xffff)
        assertEqualHexWord(remainder, 0)
    end
end
