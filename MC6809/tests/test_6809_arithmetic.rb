...
class Test6809_Arithmetic < BaseCPUTestCase
    def test_ADDA_extended01
        ...
        assertEqual(@cpu.Z, 1)
        assertEqual(@cpu.get_cc_value(), 0x04)
        assertEqual(@cpu.accu_a.value, 0x00)
    end
    ...
    def test_ADDA_immediate
        ...
        for i in range(260)
            @cpu.set_cc(0x00) # Clear all CC flags
...
#             print i, a, excpected_value, @cpu.get_cc_info()
...
#             print i, a, excpected_value, @cpu.get_cc_info()
    ...
    def test_SUBA_indexed
        ...
        assertEqualHexByte(@cpu.accu_a.value, 0xed) # $ff - $12 = $ed
        assertEqualHexWord(@cpu.system_stack_pointer.value, 0x1235)
        
        cpu_test_run(start=0x1000, end_=nil, mem=[
            0xa0, 0xe0, # SUBA ,S+
        end
        ])
        assertEqualHexByte(@cpu.accu_a.value, 0xed - 0xff & 0xff) # $ee
        assertEqualHexWord(@cpu.system_stack_pointer.value, 0x1236)
    end
...
#             print i, r, excpected_value, @cpu.get_cc_info()
...
#        )
        ...
        assertEqual(@cpu.get_cc_info(), "......V.")
    end
    
    def test_SBCA_immediate_02
        a = 0x40
        @cpu.set_cc(0xff) # CC:EFHINZVC
        @cpu.accu_a.set(a)
        cpu_test_run(start=0x1000, end_=nil, mem=[
            0x82, 0x20, # SBC
        end
...
#        )
        ...
        assertEqual(@cpu.get_cc_info(), "EFHI....")
    end
...
#        @cpu.set_cc(0x00)
    ...
    def test_DAA
        ...
        assertEqualHexByte(@cpu.accu_a.value, 0x42)
        assertEqual(@cpu.C, 1)
    end
