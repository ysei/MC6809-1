...
class Test6809_Register < BaseCPUTestCase
    ...
    def test_register_16bit_negative2
        @cpu.index_x.set(0)
        @cpu.index_x.decrement()
        assertEqualHex(@cpu.index_x.value, 0x10000 - 1)
        
        @cpu.index_x.set(0)
        @cpu.index_x.decrement(2)
        assertEqualHex(@cpu.index_x.value, 0x10000 - 2)
    end
end
...
class Test6809_CC < BaseCPUTestCase
    ...
    def test_AND
        ...
        for i in range(260)
            @cpu.accu_a.set(i)
            @cpu.set_cc(0x0e) # Set affected flags: ....NZV.
...
#             print i, r, excpected_value, @cpu.get_info, @cpu.get_cc_value()
            ...
            # test all CC flags
            if r == 0
                assertEqual(@cpu.get_cc_value(), 4)
            else
                assertEqual(@cpu.get_cc_value(), 0)
            end
        end
    end
end
...
class Test6809_Ops < BaseCPUTestCase
    def test_TFR01
        ...
        assertEqual(@cpu.index_y.value, 0) # destination
        ...
        assertEqual(@cpu.index_y.value, 512)
    end
    
    def test_TFR02
        @cpu.accu_b.set(0x55) # source
        assertEqual(@cpu.get_cc_value(), 0) # destination
        ...
        assertEqual(@cpu.get_cc_value(), 0x55) # destination
    end
    ...
    def test_TFR03
        ...
        assertEqualHex(@cpu.accu_d.value, 0x1234) # destination
    end
...
#             )
    ...
    def test_ABX_01
        ...
        assertEqualHex(@cpu.index_x.value, 0x00ff)
        ...
        assertEqualHex(@cpu.index_x.value, 0x010E)
        assertEqualHex(@cpu.get_cc_value(), 0x00)
    end
end
...
class Test6809_TestInstructions < BaseCPUTestCase
    ...
    def test_TST_direct
        for i in range(255)
            @cpu.accu_a.set(i)
            @cpu.set_cc(0xff) # Set all CC flags
...
class Test6809_Stack < BaseStackTestCase
    def test_PushPullSytemStack_01
        ...
        assertEqualHex(@cpu.accu_a.value, 0x1a)
        
        @cpu.accu_a.set(0xee)
        
        assertEqualHex(@cpu.accu_b.value, 0x00)
        ...
        assertEqualHex(@cpu.accu_a.value, 0xee)
        assertEqualHex(@cpu.accu_b.value, 0x1a)
    end
    ...
    def test_PushPullSystemStack_02
        ...
        assertEqualHex(@cpu.accu_a.value, 0xef)
        ...
        assertEqualHex(@cpu.accu_a.value, 0xef)
        assertEqualHex(@cpu.accu_b.value, 0x02)
        
        cpu_test_run(start=0x4000, end_=nil, mem=[
            0x35, 0x02, # PULS A
        end
        ])
        assertEqualHex(@cpu.accu_a.value, 0xab)
        assertEqualHex(@cpu.accu_b.value, 0x02)
    ...
    def test_PushPullSystemStack_03
        ...
        assertEqualHex(@cpu.accu_d.value, 0x5432)
        
        cpu_test_run(start=0x4000, end_=nil, mem=[
            0x35, 0x06, # PULS B,A
        end
        ])
        assertEqualHex(@cpu.accu_d.value, 0xabcd)
        assertEqualHex(@cpu.accu_a.value, 0xab)
        assertEqualHex(@cpu.accu_b.value, 0xcd)
...
class Test6809_Code < BaseCPUTestCase
    ...
    def test_code01
        ...
        assertEqualHex(@cpu.accu_a.value, 0x21)
        assertEqualHex(@cpu.accu_b.value, 0x21)
        assertEqualHex(@cpu.accu_d.value, 0x2121)
        assertEqualHex(@cpu.index_x.value, 0x2222)
        assertEqualHex(@cpu.index_y.value, 0x0000)
        assertEqualHex(@cpu.direct_page.value, 0x00)
    ...
    def test_code_LEAU_01
        ...
        assertEqualHex(@cpu.user_stack_pointer.value, 0x100)
    end
    ...
    def test_code_LEAU_02
        ...
        assertEqualHex(@cpu.user_stack_pointer.value, 0x1abc)
    end
    ...
    def test_code_LDU_01
        ...
        assertEqualHex(@cpu.user_stack_pointer.value, 0x1234)
    end
    ...
    def test_code_ORA_01
        ...
        assertEqualHex(@cpu.accu_a.value, 0x33)
        assertEqual(@cpu.N, 0)
        assertEqual(@cpu.Z, 0)
        assertEqual(@cpu.V, 0)
    end
    ...
    def test_code_ORCC_01
        ...
        assertEqualHex(@cpu.get_cc_value(), 0x33)
    end
end
...
class TestSimple6809ROM < BaseCPUTestCase
    ...
    def _is_carriage_return (a, pc)
        ...
        assertEqualHex(@cpu.program_counter.value, pc)
    end
