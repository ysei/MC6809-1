...
def bin2hexline (data, add_addr=true, width=16)
    ...
    e.g.
    File.open("C:\Python27\python.exe", "rb") do |f|
        data = f.read(150)
    end
    ...
    0080 69 73 20 70 72 6f 67 72 61 6d 20 63 61 6e 6e 6f.equal?.program.canno
    ...
    assert isinstance(data, binary_type), (
        sprintf(".equal? type: %s and not bytes/str: %s", type(data), repr(data))
    end
    )
...
def _bin2hexline_example
    ...
    File.open(sys.executable, "rb") do |f|
        data = f.read(500)
    end
